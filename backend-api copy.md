...
Rutas **no implementadas** en backend actual (no existen en el CRUD nativo de Strapi 5):
...
Usa el CRUD nativo de Strapi 5:

- `POST /api/adresses` para crear direccion
- `GET /api/adresses` + filtros por relacion real del esquema (`customer` / `users_permissions_user`) para listar las direcciones del usuario
- `PUT /api/adresses/:documentId` para actualizar
- `DELETE /api/adresses/:documentId` para eliminar
...
- En varios esquemas Strapi 5 de este proyecto, `filters[user][...]` puede fallar con `400 Invalid key user`.
- Filtros recomendados primero: `filters[customer][users_permissions_user][id][$eq]=<userId>` o `filters[users_permissions_user][id][$eq]=<userId>`.
- `filters[user][...]` dejarlo solo como fallback si tu content-type realmente define ese campo.
